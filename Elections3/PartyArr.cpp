...
const Party& PartyArr::getParty(int partyNum) const
{
	...
	throw "No party with this party id";
}
...
void PartyArr::CheckIfRep(const Citizen* newrep)const { //gets a citizen we know exists and returns if they are a rep of a party
	for (auto& party : party_arr)
	{
		if (party->getLeader().getID() == newrep->getID())
			throw"This citizen is already a representative";
